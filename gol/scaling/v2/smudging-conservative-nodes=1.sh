...
# running code
for levels in {1,3,7,15,31,63,127,255,511,1023,2047,4095,8191}; do
    # conservative
    ...
    mpirun --bind-to core -np $np \
        "$DORYTA_BIN" --synch=2 --spike-driven \
            ...
            --output-dir=$outdir --extramem=$((4000000 / np)) > $outdir/model-result.txt
    ...
    mpirun --bind-to core -np $np \
        "$DORYTA_BIN" --synch=3 --spike-driven \
            ...
            --random-spikes-time=1.0 \
            --random-spikes-uplimit=$((grid_width * grid_width)) \
            --output-dir=$outdir --extramem=$((8000000 / np)) > $outdir/model-result.txt
done
